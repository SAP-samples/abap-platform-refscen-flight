...
CLASS ltcl_tdf_buffer IMPLEMENTATION.
  ...
  METHOD supplement_create_text_update.
    ...
    READ ENTITIES OF /dmo/c_supplement
      ENTITY supplement
        FIELDS ( supplementcategory price currencycode supplementdescription )
        WITH CORRESPONDING #( mapped-supplement )
        RESULT DATA(c_supplements)
      FAILED    failed
      REPORTED  reported.
    ...
    cl_abap_unit_assert=>assert_not_initial( c_supplements ).
    cl_abap_unit_assert=>assert_equals( exp = 1
                                        act = lines( c_supplements ) ).

    c_supplement_act = CORRESPONDING #( c_supplements[ 1 ] EXCEPT %key ).
    c_supplement_exp = CORRESPONDING #( c_update EXCEPT %key supplementcategory ).
    c_supplement_exp-supplementcategory = c_create-supplementcategory.

    cl_abap_unit_assert=>assert_equals( exp = c_supplement_exp
                                        act = c_supplement_exp ).
