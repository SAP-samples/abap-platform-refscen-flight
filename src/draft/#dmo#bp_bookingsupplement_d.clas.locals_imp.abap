...
CLASS lhc_bookingsupplement IMPLEMENTATION.
  ...
  METHOD validateSupplement.
    ...
    DELETE lt_booksuppl WHERE SupplementID IS INITIAL.
    CHECK lt_booksuppl IS NOT INITIAL.

    SELECT FROM /dmo/supplement FIELDS supplement_id
                                FOR ALL ENTRIES IN @lt_booksuppl
                                WHERE supplement_id = @lt_booksuppl-SupplementID
           INTO TABLE @DATA(lt_supplement).
    ...
    LOOP AT lt_booksuppl ASSIGNING FIELD-SYMBOL(<fs_booksuppl>).
      ...
      ELSEIF <fs_booksuppl>-SupplementID IS NOT INITIAL AND NOT line_exists( lt_supplement[ supplement_id = <fs_booksuppl>-SupplementID ] ).
        APPEND VALUE #(  %tky = <fs_booksuppl>-%tky ) TO failed-bookingsupplement.
